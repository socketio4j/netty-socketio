...
public class DistributedRedissonClusterTest {
    ...
    private static final String STREAM_SINGLE_CHANNEL_PREFIX = "STREAM_SINGLE_";
    private static final String STREAM_MULTI_CHANNEL_PREFIX = "STREAM_MULTI_";
    ...
    class StreamSingleChannelTest extends DistributedCommonTest {
        ...
        void setupNodes() throws Exception {
            ...
            cfg1.setPort(findAvailablePort());
            cfg1.setStoreFactory(new RedisStoreFactory(redisClient1,
                    new RedisStreamEventStore.Builder(redisClient1).eventStoreMode(EventStoreMode.SINGLE_CHANNEL).prefix(STREAM_SINGLE_CHANNEL_PREFIX).build()));
            node1 = new SocketIOServer(cfg1);
            DistributedClusterIntegrationSupport.attachDefaultRoomListeners(node1);
            node1.start();
            port1 = cfg1.getPort();
            ...
            cfg2.setPort(findAvailablePort());
            cfg2.setStoreFactory(new RedisStoreFactory(redisClient2,
                    new RedisStreamEventStore.Builder(redisClient2).eventStoreMode(EventStoreMode.SINGLE_CHANNEL).prefix(STREAM_SINGLE_CHANNEL_PREFIX).build()));
            node2 = new SocketIOServer(cfg2);
            DistributedClusterIntegrationSupport.attachDefaultRoomListeners(node2);
            node2.start();
            port2 = cfg2.getPort();
    ...
    class StreamMultiChannelTest extends DistributedCommonTest {
        ...
        void setupNodes() throws Exception {
            ...
            cfg1.setPort(findAvailablePort());
            cfg1.setStoreFactory(new RedisStoreFactory(redisClient1,
                    new RedisStreamEventStore.Builder(redisClient1).eventStoreMode(EventStoreMode.MULTI_CHANNEL).prefix(STREAM_MULTI_CHANNEL_PREFIX).build()));
            node1 = new SocketIOServer(cfg1);
            DistributedClusterIntegrationSupport.attachDefaultRoomListeners(node1);
            node1.start();
            port1 = cfg1.getPort();
            ...
            cfg2.setPort(findAvailablePort());
            cfg2.setStoreFactory(new RedisStoreFactory(redisClient2,
                    new RedisStreamEventStore.Builder(redisClient2).eventStoreMode(EventStoreMode.MULTI_CHANNEL).prefix(STREAM_MULTI_CHANNEL_PREFIX).build()));
            node2 = new SocketIOServer(cfg2);
            DistributedClusterIntegrationSupport.attachDefaultRoomListeners(node2);
            node2.start();
            port2 = cfg2.getPort();
